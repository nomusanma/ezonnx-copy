...
from ezonnx.data_classes.ocr import BoxedResult
...
class TextSystem:
    def __init__(self, 
                 sess_det, sess_rec, sess_cls,
                 dict_path:str,
                 use_angle_cls=False, 
                 box_thresh=0.6, 
                 unclip_ratio=1.6):
        self.text_detector = TextDetector(sess_det,
                                        box_thresh=box_thresh,
                                        unclip_ratio=unclip_ratio,
                                        )
        self.text_recognizer = TextRecognizer(sess_rec,dict_path)
        self.use_angle_cls = use_angle_cls
        if self.use_angle_cls:
            self.text_classifier = TextClassifier(sess_cls)
    ...
    def detect_and_ocr(self, img, drop_score=0.5, unclip_ratio=None, box_thresh=None):
        ...
        for box, rec_reuslt, img_crop in zip(dt_boxes, rec_res, img_crop_list):
            ...
            if score >= drop_score:
                res.append(BoxedResult(box=box, img=img_crop, text=text, score=score))
        return res
