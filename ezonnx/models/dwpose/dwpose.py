...
class DWPose(Inferencer):
    ...
    def __call__(self,image:Union[str, np.ndarray])-> PoseDetectionResult:
        ...
        for box in boxes:
            kpt,kpt_scores = self.estimate_keypoints(box,image)
            if 'kpts' in locals():
                kpts = np.vstack([kpts,kpt])
            else:
                kpts = kpt
            if 'kpt_scores' in locals():
                kpt_scores = np.vstack([kpt_scores,kpt_scores])
            else:
    ...
    #         img (np.ndarray): Input image in shape.
    #         scale(float): scale to restore cordinates to original

    #     Returns:
    #         outputs (np.ndarray): Output of RTMPose model.[simcc_x, simcc_y]
    ...
    #     # h, w = self.sess_det.get_inputs()[0].shape[2:]
    #     # model_input_size = (w, h)
    #     # リサイズ
    #     resized_frame, center, scale = self._preprocess(frame, 
    #                                                    self.input_size_det)
    #     # 人物検出
    #     outputs_onnx = self.inference(self.sess_det, resized_frame)

    #     # 元サイズでの座標に戻す
    #     outputs = convert_resized_bbox_to_original(outputs_onnx,
    #                                                 frame.shape,
    #                                                 resized_frame.shape)
    #     # bboxとスコア
    ...
    #     # 閾値以下のbbox除外
    #     bboxes = bboxes[scores>=self.det_thresh]
    #     classes = classes[scores>=self.det_thresh]
    #     scores = scores[scores>=self.det_thresh]

    #     # nms
    #     bboxes,scores,classes = nms(bboxes.tolist(),scores.tolist(),classes.tolist(),iou_threshold=0.5)
